...
def get_status_badge(date_str: str, time_str: str) -> str:
    """Возвращает статус-значок в зависимости от текущего времени."""
    try:
        event_dt = datetime.strptime(f"{date_str} {time_str}", "%d.%m.%Y %H:%M")
        now = datetime.now()

        if now >= event_dt:
            return "🔴 <b>Идёт сейчас!</b>\n\n"
        else:
            diff = event_dt - now
            days = diff.days
            if days == 0:
...
def build_card(event) -> str:
    free_seats = event["free_seats"] if event["seats"] > 0 else "Неограниченно"
    status_badge = get_status_badge(event["date"], event["time"])
